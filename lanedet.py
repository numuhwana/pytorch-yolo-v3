...
cap = cv2.VideoCapture('new.mp4')#('challenge.mp4')
...
def draw_fitline(img, result_l, result_r, color=(0, 0, 255), thickness=10):
    # draw fitting line
...
def erase_outliers(par, lines):
    ...
    filtered_lines = lines[distance < 10, :]
...
def detect_lanes_img(img):
    ...
    line_arr = hough_lines(canny_img, 1, 1 * np.pi / 180, 30, 9, 20)
